...
`DISABLED` telemetry and a `DIRTY` worktree are observations, not failures by themselves. Exit `0` means healthy, `1` means action is required, and `2` means doctor could not complete a check.
